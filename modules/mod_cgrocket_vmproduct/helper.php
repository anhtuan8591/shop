...
abstract class modnsrocketJHelper
{
	static function getList($params,$count){
		...
		foreach ($items as &$item) {
			...
			$item->image 		= modnsrocketJHelper::getImage($item->introtext,$item->images);
			$item->title 		= htmlspecialchars($item->title);
			$item->introtext 	= JHtml::_('content.prepare', $item->introtext);
			$item->link 		= JRoute::_(ContentHelperRoute::getArticleRoute($item->slug, $item->catslug));
			$item->comment 		= modnsrocketJHelper::getComment($item->link, $item->catid, $plgParams);
		...
		if (JPluginHelper::isEnabled('content', 'spcomments')) {
			if (in_array($catid, $params->get('catids'))) {	
				...
				if ($commenting_engine=="disqus") {//if disquss
					$link = '<a class="rocket-comments" href="' . $url . '#disqus_thread" data-disqus-identifier="' . $identifier . '"></a>';
					...
					$link = "<a class=\"rocket-comments\" href='$url'>Comments (<fb:comments-count href='$url'></fb:comments-count>)</a>";
				}
				return $link;
			}	
		}
