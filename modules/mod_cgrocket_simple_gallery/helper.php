...
abstract class modCGGalleryHelper {
	...
	static function getimgList($params) {
		...
		foreach ($files as $file) {
			$image				= modCGGalleryHelper::getImages($path.'/'.$file,$thumbwidth,$thumbheight,$thumbratio);
	...
	static function getImages($image, $thumbwidth=100, $thumbheight=100, $thumbratio) {	  
		...
		if (isset($image)) {
			...
			if (function_exists("gd_info")) {
				...
				if (!class_exists("spThumbnail")) include_once('class.cgThumbnail.php');
