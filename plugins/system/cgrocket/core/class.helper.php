...
class cgrocketHelper {
    ...
    function __construct($_this){
		...
		$this->cgrocketPath=JPATH_PLUGINS .DS. 'system' .DS.'cgrocket';		
		$this->cgrocketUrl= $this->baseUrl . '/plugins/system/cgrocket';
    }
	...
	function import($path) {//import file
		...
		} elseif(JFile::exists($this->cgrocketPath . DS . $filepath)) { //if not found, then check from cgrocket path
           require_once($this->cgrocketPath . DS . $filepath);
		} 
	}
    ...
    function loadLayout() {//load mainbody layout
		...
		$this->API->addStyleDeclaration('.cg-wrap {width: ' . $main_width . 'px;}
		#cg-leftcol {width: ' . $left_width . $unit .'}
		#cg-rightcol { width: ' . $right_width . $unit .'}
		#cg-maincol {width:' . $content_width . $unit .'}');	
		$this->API->addStyleDeclaration('#inner_content {width: ' . $inner_width . $unit .';}
		#cg-inset1 {width: ' . $inset1 . $unit .'}#cg-inset2 { width: ' . $inset2 . $unit .'}');	
		
		//Equal Height - added version 1.9
		if ($fixed_height) {
			$this->addJS('equalheight.js'); 
			$this->addInlineJS('function fixHeight_main () {
				var equalizer_main = new Equalizer("#cg-leftcol, #cg-maincol, #cg-rightcol").equalize("height");
			}
	...
	function addJS ($srcs) {//add javascript
		...
		foreach ($srcs as $src) {
			...
			} elseif(JFile::exists($this->cgrocketPath . '/js/' . $src)) { //if not found, then check from cgrocket path
			   $this->API->addScript($this->cgrocketUrl . '/js/' . $src);
			}		
		}
	}
	...
	function addCSS ($srcs) {//Add stylesheets
		...
		foreach ($srcs as $src) {
			...
			} elseif(JFile::exists($this->cgrocketPath . '/css/' . $src)) { //if not found, then check from cgrocket path
			   $this->API->addStyleSheet($this->cgrocketUrl . '/css/' . $src);
			}		
		}
	}	
	...
	function renderModules($mods,$style='cg_raw',$class='') {//Load modules
		$output='';
		foreach($mods as $mod) {
			$output	.='<div style="width:' . $mod['width'] . '" class="cg-block '. $mod['sep'] . '">';
	 ...
	 * @since	1.9
	 */
	function addModules($modPos, $style='cg_raw', $gridId='', $width='', $equalHeight = false, $fluid = false) {
		$output='';
		...
		if ($mods) {
			...
			if ($fluid) {//full width
				$output .= '<div class="cg-wrap clearfix">';
			}
			
			if ($gridId || $fluid) {
				$output .= '<div class="cg-inner clearfix">';
			}
			
			foreach($mods as $mod) {
				if ($gridId) {
					$output	.= '<div style="width:' . $mod['width'] . '" class="cg-block '. $mod['sep'] . '">';
					$output	.= '<div id="cg-' . $mod['name'] . '" class="mod-block cg-' . $mod['name'] . ' ' . $mod['extra-css'] . '">';
				} else {
					$output	.= '<div id="cg-' . $mod['name'] . '" class="clearfix">';			
					$output	.= '<div class="cg-inner clearfix">';
				}
				$output	.= '<jdoc:include type="modules" name="' . $mod['name'] . '" style="' . $style . '" />';
				$output	.= '</div>';	
	 ...
	 */
	function getMenu() {
		$menutype  = isset($_COOKIE[$this->theme . '_menu']) ? $_COOKIE[$this->theme . '_menu'] : $this->API->params->get('cgmenu','css');
		if(isset($_GET['menu'])) {
	...
	function loadMenu () {
		...
		return new cgrocketMenu ($this->API->params,$this->getMenu());
	}
    ...
    function compressJS() {//function to compress js files
		...
		$diff=false;
		$cgrocket_folder='cgrocket_assets';//path of cache where to save
        $output = array();
        $md5sum = null;
		...
		if (is_writable(JPATH_CACHE)) {//add cgrocket_assets folder under cache directory
			if (!file_exists(JPATH_CACHE.DS.$cgrocket_folder)) mkdir (JPATH_CACHE.DS.$cgrocket_folder);
		}

        if (count($js_files) > 0) {//if any js file available
            $cache_name = md5($md5sum) . ".js";
            $cache_path = JPATH_CACHE . DS . $cgrocket_folder . DS . $cache_name;
            ...
            $cache_url = $this->API->baseurl . "/cache/" . $cgrocket_folder . '/' . $cache_name;//path of css cache to add as script
	...
	function compressCSS() {//function to compress css files
        ...
        if (count($css_files) > 0) {//if any css file available
            ...
            $cache_url = $this->API->baseurl . "/cache/" . $cgrocket_folder . '/' . $cache_name;//path of css cache to add as stylesheet
